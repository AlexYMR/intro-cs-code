...
TEST_CASE("Initiating test..."){
	SECTION("Testing the CompletelyFair class functions"){
		...
		CompletelyFair* ioProcTree = new CompletelyFair;
		...
		delete ioProcTree; //probably causes a memory leak (loses pointers to blocked process)

		CompletelyFair* cpuProcTree = new CompletelyFair;
	...
	SECTION("Testing the FastCompletelyFair class functions"){
		cout << "\nTesting FastCompletelyFair class functions...";

		FastCompletelyFair* ioProcTree = new FastCompletelyFair;
		//make an ioboundproc tree = {1,2,3}, where 1 and 3 are unblocked
		//and 2 is blocked
		int sliceSize = 1;
		int curCycle1 = 1;
		...
		FastCompletelyFair* cpuProcTree = new FastCompletelyFair;
