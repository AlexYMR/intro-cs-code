...
# // By Alex Malave and Lauren Kranis
# -------------------------------------
...
class PuzzleGui(Frame):
    ...
    def __initUI(self):
        ...
        clear_button = Button(self,text="Clear answers",command=self.clearAnswers) #--Sends the text the button will display, along with the function it will call when clicked
        clear_button.pack(fill=BOTH, side=BOTTOM) #--Fills the space, and is placed at the bottom of the window

        self.drawGrid()
        self.drawPuzzle()

        self.canvas.bind("<Button-1>", self.cellClicked) #--Binds an action (clicking the left mouse button to a function)
        self.canvas.bind("<Key>", self.keyPressed) #--Fired when a key is pressed (e.g. a number [for input])
    ...
    def clearAnswers(self):
        ...
        self.drawPuzzle() #--Draws the puzzle again with the newly reset values
		
    def cellClicked(self, arg):
    ...
    def keyPressed(self, arg):
        ...
        if self.row >= 0 and self.col >= 0 and arg.char in "1234567890": #--If the key pressed is a character in the string (note: arg.char returns an ordinal value)
            ...
            self.drawPuzzle() #--Redraws the puzzle as it now is
            self.drawCursor() #--Redraws the cursor (now it shouldn't be selecting anything)
